			...
			<form method="post" id="editpage" enctype="multipart/form-data">
						...
						<div class="form-group row">
										...
										<div class="form-group row">
											<div class="col-12">
										      <div class="form-group row">
										        ...
										        <div class="col-8">
										          <textarea id="edescription" name="edescription" cols="40" rows="5" class="form-control"><?=$projects['edescription']?></textarea>
										        </div>
										      </div>
											</div>
										</div>
							...
							<div class="col-6">
								<div class="form-group row">
									...
									<div class="col-6">
										<img class="col-md-8 border border-danger" id="uploadimage" src="<?=file_exists("assets/background/".$projects['id'].".jpg")?base_url('assets/background/'.$projects['id'].'.jpg'):base_url('assets/plus.png')?>">
									</div>
								</div>
								<style type="text/css">
									input[type="file"] {
				...
				<div class="form-group row">
					<div class="offset-4">
					  ...
					  <a href="<?=base_url('home/managestudio/background')?>" name="submit" class="btn btn-danger"><?=$language[$this->session->userdata('language')]['upload.cancel']?></a>
					</div>
				</div>
			</form>
			<script type="text/javascript">
				...
				$('#editpage').on('submit', function(e){
					...
					{  
						...
						$.ajax({  
							url:"<?=base_url('home/singleImageStore/background?id='.$projects['id'])?>",
							method:"POST",  
							data:form_data,  
							contentType: false,  
							cache: false,  
					...
					}
					{
				        ...
				        $.ajax({
				        	url: "<?=base_url('home/mbsaveclick?id='.$projects['id'])?>",
				        	method:"POST",
				        	data:form_data,
				        	success:function(res) 
				        	{
								...
								window.location.href = "<?=base_url('home/managestudio/background')?>";
				        	}
				        });
					}
				}); 
